...
- [discipline-witnesses-naturalist-taxonomy-recursion-2026-05-19.md](captures/discipline-witnesses-naturalist-taxonomy-recursion-2026-05-19.md) — team-naturalist systematic validation of scout's "8-class taxonomy recurses at attestation tier" claim. Scout cited 4 of 8 classes; this capture checks all 8 and finds ALL 8 have attestation-tier analogs in v3 primitives (Frame-translation ↔ `signed_against_fingerprint`; Forgotten-lesson ↔ `discipline_doc`+`ratified_doc`; Implicit-coupling ↔ `descended_from` propagation w/ F10 inheritance hint; Stale-context ↔ `fresh_within_days`; Premature-abstraction ↔ `evidence_provenance`; Incompatible-merger ↔ `RatificationKind` discriminator + T4-A; Boundary-violation ↔ witness-provider-crate enforcement; Optionality-collapse ↔ `signers(against=...)` + delta-chain anti-laundering). Scout's claim is NOT cherry-picked; systematic. Biology grounding: immune systems exhibit recursive recognition (Tregs recognize T-cells; anti-idiotype antibodies recognize antibodies) using same machinery at multiple architecture levels. CONSERVATIVE SCOPING: two-substrate recurrence (code + attestation) is evidence for the pattern; substrate-INDEPENDENCE is a stronger claim needing more substrates checked. Ready-to-fold ADR-019 text in capture's 'Implication for ADR-019' section names every v3 primitive by which class it fights. Supports framing-B further
