...
use antigen::learn::curate::{CurationAction, apply, curate};
use antigen::learn::discriminator::{ClassVerdict, classify};
...
fn atk_curate3_double_retire_corrupts_autobiography() {
    ...
    assert_eq!(
        first,
        Some(LifeEvent::Retired),
        "first Forget must record Retired"
    );
...
fn atk_curate5_reversible_actions_never_retire() {
    for verdict in [
        ...
        assert!(
            !rec.is_retired(),
            "ATK-CURATE-5a: {verdict:?} must never retire"
        );
        assert_eq!(
            rec.events().len(),
            before,
...
fn atk_curate7_shape_gone_but_defended_keeps_and_leaves_history_clean() {
    ...
    assert!(
        !rec.is_retired(),
        "ATK-CURATE-7a: a defended class is never retired"
    );
    assert_eq!(
        rec.events().len(),
        before,
